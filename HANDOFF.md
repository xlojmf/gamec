...
- **`setActivePlayers({ value })` locks the current player out of ALL moves** until every stage player calls `events.endStage()` — used by the trade `respond` stage (and the `discard` stage) to block the turn while someone else must act. Consequence: while a trade is pending NOTHING broadcasts, so any client that misses the proposal broadcast or gets rolled back by a stale sync stays stuck until a refresh — this is the trade-freeze class of bug, now fixed by queued syncs (see above).
- **The client applies `sync` responses UNCONDITIONALLY (no stateID guard)** and its store then carries a regressed `_stateID`, making every subsequent move fail the server's stale-stateID check. `update` broadcasts DO have a guard. Never trigger mid-game syncs; guard against stale ones (`staleSyncGuard`). Moves using `events`/`random` are NOT applied optimistically on the client (`noClient`) — they round-trip through the server broadcast.
...
## September 12 fixed map presets (tournament islands)

- **Feature:** online rooms can now start on a FIXED island — `POST /rooms` takes `map` (preset id), stored on the room (`StoredRoom.mapPreset`, survives rematch), threaded through bgio `setupData.mapPreset` → `G.mapPreset` → every `boardFor(G.seed, G.mapPreset)` / `generateBoard(seed, preset)` call (client validators, GameView, watchdog). Hotseat: `/game?map=<id>`. The room page and game HUD show the map name; the lobby picker (`MapPreview`) renders each island as an SVG mini-map.
- **Presets:** the four **Catan World Championships 2025** maps (`cwc2025-1..4`, `src/game/maps.ts`) — terrain reconstructed from catancollector.com's published renders by a CV pipeline (scratch: `work/maps/extract3.mjs`): detect the sea annulus + island hexagon in the tilted render, DLT-homography to the engine's canonical top-down island, sample a chit-avoiding annulus per hex, then GLOBAL rank assignment (fixed 4/4/4/3/3/1 composition — greens split by brightness, warm tiles ranked by orangeness, chit-less tile = desert). The tour/uscnc renders are a different style the pipeline doesn't calibrate against — skipped deliberately.
- **Numbers** follow the official A–R chit rule (spiral, skipping the desert) — note official fixed maps can carry adjacent 6/8 (CWC #1/#3 do; the no-adjacent-red rule is variable-setup only). **Ports stay seeded** even on fixed islands (the fixed part is the terrain).
- Tests: `maps.test.ts` (registry validity), `board.test.ts` (preset terrain verbatim, A–R multiset, determinism), `mp.e2e.test.ts` (room with map → every client board matches the preset; unknown ids fall back to random). Gates: typecheck ✅ 157/157 ✅ builds ✅ built-server smoke ✅.

## September 12 robber standee + full soundboard

- **3D robber piece is now the golden knight:** `addRobber` builds a dark plinth + the knight art as a standee cutout (plane, alphaTest, golden emissiveMap so it glows) that yaw-billboards to the camera between hops (`updateRobber`). The classic dark pawn stays as the fallback until/unless the art loads; hop spin unchanged. The pawn/standee swap is per-GameView, texture cached module-level (`loadKnightStandeeTexture`).
- **All 30 sound files are wired** (exhaustively verified against `public/sounds`): automatic events gained — 7 rolled → *Pega ladrão!*, steal → *Seu Madruga*, discard submission → *Saw “I want to play a game”*, wood/wool/grain production → *Lumberjack/Sheep/Cereal* (production beats the generic dice rattle), Monopoly → rickroll, knight & trade acceptances alternate voices deterministically (sequence/length-hash), multiplayer loss → *Undertaker's bell* (local win still cheers). The rest (Golo, peons, CP Comboios, funeral march, bomb…) are manual cues in the soundboard select; quick-grid now March / Knight / Dice / Robber / City. `SoundEvents` gained `seven`, `production`, `lost` props (game.tsx).
- Gates: typecheck ✅ · 146/146 tests ✅ · builds ✅ · preview smoke (new MP3s + knight.png serve 200) ✅.

## September 12 knight art & harbor sign readability

- **Knight art:** user-supplied `public/knight.png` (golden knight on transparency) was downscaled 793×1760→324×720 (1MB→256KB) into `public/assets/astra/ui/knight.png` (registered in manifest `ui.knight`; original kept at ignored `work/knight-original.png`). New `KnightArt` + `DevCardIcon` components (`GameIcon.tsx`) give Knights real art everywhere they appear larger than a glyph: played-card announcement (124px), HandTray dev-card chips, right-panel dev rows, soundboard. Tiny glyphs keep the engraved SVG. Sizing lives in `.knight-art` context rules (game-ui.css).
- **Harbor signs:** 2:1 port medallions now show the traded resource's actual icon instead of a written label — the UI SVGs are fetched, width/height-injected and drawn onto the sign canvas (async; texture `needsUpdate`s in place, cached per kind). Canvas 256→384, medallion radius 0.24→0.28, ratio text larger, anisotropy 8. Generic harbors keep “3:1 ANY”. The sidebar harbor legend uses the real `ResourceIcon`s too (swatch only for the generic 3:1).
- Gates: typecheck ✅ · 140/140 tests ✅ · web + game-server builds ✅ · preview smoke: page, knight.png and all port SVGs serve 200 ✅.

## September 11 trade-freeze fix (sync/broadcast race + offer privacy)

- **Root cause of the stuck player trades:** bgio 0.50's socket transport handles `sync` requests OUTSIDE the per-match FIFO queue that serializes moves. Any mid-game re-sync (socket reconnect is enough) opened two windows: (1) the socket is unregistered from the match broadcast list during the async sync — a trade proposal broadcast in that window is simply never delivered; (2) the sync's db fetch can resolve before the concurrent move's write commits, so the client receives a full-state snapshot OLDER than a broadcast it already applied — 0.50's SYNC reducer replaces state unconditionally (no stateID guard), wiping the pending trade AND regressing the client's stateID so every later move fails the server's `invalid stateID` check. A pending trade blocks all moves → no further broadcast ever corrects the rolled-back client → the table freezes until a manual refresh (which re-syncs and shows the offer, exactly as reported).
- **Server fix:** `queuedSyncTransport()` in `gameServer.ts` patches bgio's SocketIO transport so every `sync` runs through the same per-match queue as `update` (socket.io namespace middleware wraps `socket.on('sync')` before bgio's connection listeners register). Wire is now strictly stateID-ordered. `startGameServer` accepts `opts.db` for deterministic e2e racing.
- **Client fix:** `src/multiplayer/transport.ts` `staleSyncGuard()` wraps the multiplayer factory and drops sync payloads older than anything already seen — belt-and-braces against any remaining stale-sync path (unit-tested).
- **Regression e2e:** `mp.trade.e2e.test.ts` boots the server with a write-delayed FlatFile, fires a re-sync the instant a client applies a move broadcast, and asserts no rollback. With the transport patch temporarily disabled it reproduces the exact production failure (`invalid stateID, was=[0], expected=[1]` + frozen match), proving the test bites.
- **Offer privacy (rules):** `proposeTrade` no longer filters recipients by whether they can cover `take` — the offer must be shown to everyone selected so nobody can infer a player lacks a resource from the offer not arriving; `acceptTrade` remains the legality gate. Bonus: proposer-side client validation is now consistent with the server (previously every mp propose ran INVALID locally against masked hands and errored in the console). The response overlay disables Accept with a hint when the responder can't cover the cards but keeps Decline/Counter.
- Validation: typecheck ✅, **140/140 tests** (5× full-suite runs), web + game-server builds ✅, built-server smoke (queued initial sync + mid-connection re-sync) ✅.
